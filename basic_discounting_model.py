"""
script for mdp for assignment submission problem: There is an initial state
(when assignment is not started), potential intermediate states, and final
state of completion. At each non-completed state, there is a choice between
actions to WORK which has an immediate effort cost and SHIRK which has an
immediate reward. The final state is absorbing and also has a reward (usually
set equivalent to rewards from shirk). The outcome from evaluation only
comes at the deadline (which can be negative to positive based on state at
final timepoint).
"""
...
start_works = np.full(
    (len(efficacys), N_INTERMEDIATE_STATES+1, len(rewards_task)), np.nan)
...
for i_reward_task, reward_task in enumerate(rewards_task):
    ...
    for i_efficacy, efficacy in enumerate(efficacys):
        ...
        for i_state in range(N_INTERMEDIATE_STATES+1):
            # find timepoints where it is optimal to work
            # (when task not completed, state=0
            start_work = np.where(policy_opt[i_state, :] == 0)[0]

            if len(start_work) > 0:
                # first time to start working
                start_works[i_efficacy, i_state, i_reward_task] = start_work[0]

for i_state in range(N_INTERMEDIATE_STATES+1):

    plt.figure(figsize=(8, 6))
    for i_reward_task, reward_task in enumerate(rewards_task):
        plt.plot(efficacys, start_works[:, i_state,
                 i_reward_task], label=f'{reward_task}:0.5')
    plt.xlabel('efficacy')
...
for i_reward_task, reward_task in enumerate(rewards_task):
    ...
    for i_effort, effort_work in enumerate(efforts):
        ...
        for i_state in range(N_INTERMEDIATE_STATES+1):
            ...
            if len(start_work) > 0:
                # first time to start working
                start_works[i_effort, i_state, i_reward_task] = start_work[0]
                # print( policy_opt[0, :])
...
for i_state in range(N_INTERMEDIATE_STATES+1):
    ...
    for i_reward_task, reward_task in enumerate(rewards_task):
        plt.plot(efforts, start_works[:, i_state,
                 i_reward_task], label=f'{reward_task}:0.5')
...
plt.savefig(
    'writing/figures_thesis/vectors/basic_case_trajectory_no_discount.svg',
    format='svg', dpi=300
)
...
for i_efficacy, efficacy in enumerate(efficacys):
    ...
    for i_state in range(N_INTERMEDIATE_STATES+1):
        ...
        if len(start_work) > 0:
            # first time to start working
            start_works[i_efficacy, i_state] = start_work[0]

    # run forward (N_runs no. of times), count number of times task is finished
    # for each policy
    initial_state = 0
    for i in range(N_runs):

        s, a, v = mdp_algms.forward_runs(policy_opt, V_opt, initial_state,
                                         HORIZON, STATES, T)
        if 2 in s:
...
ax2.set_ylabel('avg completion time', color='tab:blue',
...
for i_r, reward in enumerate(rewards):
    ...
    reward_func, reward_func_last = get_reward_functions(
        STATES, reward, REWARD_FAIL, REWARD_SHIRK,
        REWARD_COMPLETED, EFFORT_WORK, EFFORT_SHIRK
    )
    T = get_transition_prob(STATES, EFFICACY)
    V_opt, policy_opt, Q_values = mdp_algms.find_optimal_policy(
...
effort_work = -0.4
reward_shirk = 0.5
reward_func, reward_func_last = get_reward_functions_immediate(
    STATES, reward_work, reward_shirk,
    effort_work, EFFORT_SHIRK
)
T = get_transition_prob(STATES, EFFICACY)
...
for i_efficacy, efficacy in enumerate(efficacys):
    ...
    V_opt, policy_opt_imm, Q_values = mdp_algms.find_optimal_policy_prob_rewards(
        STATES, ACTIONS, HORIZON, DISCOUNT_FACTOR,
        reward_func, reward_func_last, T
    )
    ...
    for i in range(N_runs):
        ...
        s, a, v = mdp_algms.forward_runs(
            policy_opt, V_opt, initial_state, HORIZON, STATES, T)
        if s[-1] == len(STATES)-1:
            count_opt[i_efficacy, 0] += 1

        s, a, v = mdp_algms.forward_runs(
            policy_opt_imm, V_opt, initial_state, HORIZON, STATES, T)
        if s[-1] == len(STATES)-1:
            count_imm[i_efficacy, 0] += 1

plt.figure(figsize=(5, 4), dpi=100)
plt.bar(efficacys, count_imm[:, 0]/N_runs, alpha=0.5,
        width=0.1, color='tab:blue', label='immediate rewards')
plt.bar(efficacys, count_opt[:, 0]/N_runs, alpha=1,
        width=0.1, color='tab:blue', label='delayed rewards')
plt.xlabel('efficacy')
