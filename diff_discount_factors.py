"""
script for mdp for assignment submission problem with different discount
factors for costs and rewards, and immediate rewards. Again, there is an
initial state (when assignment is not started), potential intermediate states,
and final state of completion. At each non-completed state, there is a choice
between actions to DO which has an immediate effort cost and also reward if
succesful and DONT which has an immediate reward. The final state is absorbing
and also has a reward (usually set equivalent to rewards from shirk).
"""
...
V_opt_full, policy_opt_full, Q_values_full = (
    mdp_algms.find_optimal_policy_diff_discount_factors(
        STATES, ACTIONS, ACTIONS, HORIZON, DISCOUNT_FACTOR_REWARD,
        DISCOUNT_FACTOR_COST, reward_func, cost_func, reward_func_last,
        cost_func_last, T)
)

# actual policy followed by agent
effective_policy = np.array([policy_opt_full[HORIZON-1-i][0][i]
                            for i in range(HORIZON)])
# %%
# policy evaluation to get positive and negative values associated with the
# optimal policy, seprately
V_r = []
V_c = []
for i_timestep in range(HORIZON-1, -1, -1):
    ...
    v_r, v_c = mdp_algms.policy_eval_diff_discount_factors(
        STATES, i_timestep, HORIZON, reward_func_last, cost_func_last,
        reward_func, cost_func, T, DISCOUNT_FACTOR_REWARD,
        DISCOUNT_FACTOR_COST, policy_opt_full[i_timestep].astype(int)
    )

    V_r.append(v_r[0, HORIZON-i_timestep-1])
    V_c.append(v_c[0, HORIZON-i_timestep-1])
# %%
# THIS WORKS ONLY FOR A SMALL MDP WHERE ONLY ONE STATE HAS TWO CHOICES OF
# ACTIONS WITH A SMALL HORIZON
...
V_opt_bf, policy_opt_bf = (
    mdp_algms.find_optimal_policy_diff_discount_factors_brute_force(
...
# solve for common discount case
reward_func, reward_func_last = get_reward_functions_common(
    STATES, REWARD_DO, EFFORT_DO, REWARD_COMPLETED, COST_COMPLETED
)
T = get_transition_prob(STATES, EFFICACY)
V_opt, policy_opt, Q_values = mdp_algms.find_optimal_policy(
...
for i_d_r, discount_factor_cost in enumerate(discounts_cost):
    ...
    # the real optimal returns for each timestep and state
    v_opt_real = np.array([V_opt_full[HORIZON-1-i][:][:, i]
                          for i in range(HORIZON)])
    # real returns for both options in state=0
    q_val_real = np.array([Q_values_full[HORIZON-1-i][0][:, i]
                          for i in range(HORIZON)])
    # actual policy followed by agent
    effective_policy = np.array([policy_opt_full[HORIZON-1-i][0][i]
                                for i in range(HORIZON)])
    ...
    for i in range(HORIZON):
        ...
        # policy_opt_full[HORIZON-1-i][0, :-1]-policy_opt_full[HORIZON-1-i]
        # [0, 1:] == 1 , w[0]+1
        w = np.where(policy_opt_full[HORIZON-1-i][0, :-1] -
                     policy_opt_full[HORIZON-1-i][0, 1:] == 1)[0]
        if w.size > 0:
            work[i] = w[0] + 1

    axs1.plot(
        work,
        label=f"$\gamma_c$ = {discount_factor_cost}, $\gamma_r$ = {DISCOUNT_FACTOR_REWARD}",
        color=colors[i_d_r], linestyle='--')

    axs2.plot(effective_policy,
              label=f"$\gamma_c$ = {discount_factor_cost}, $\gamma_r$ = {DISCOUNT_FACTOR_REWARD}",
              color=colors[i_d_r], linestyle='--')

axs1.hlines(np.where(policy_opt[0, :] == 0)[0][0], 0, 9,
            label=f"$\gamma_r$ = $\gamma_c$ = {DISCOUNT_FACTOR_COMMON}",
            color='tab:red')
...
effective_policy[0, :] = np.array([policy_opt_full[HORIZON-1-i][0][i]
                                   for i in range(HORIZON)])
effective_policy[1, :] = np.array([policy_opt_full[HORIZON-1-i][1][i]
                                   for i in range(HORIZON)])

for i in range(N_runs):

    s, a, v = mdp_algms.forward_runs(
        effective_policy, V_opt, initial_state, HORIZON, STATES, T)
...
V_opt_full, policy_opt_full, Q_values_full = (
    mdp_algms.find_optimal_policy_diff_discount_factors(
        STATES, ACTIONS, HORIZON, DISCOUNT_FACTOR_REWARD,
        DISCOUNT_FACTOR_REWARD, reward_func, cost_func, reward_func_last,
        cost_func_last, T)
)
...
for i in range(N_runs):
    ...
    s, a, v = mdp_algms.forward_runs(
        policy_precommit, V_opt, initial_state, HORIZON, STATES, T)
...
# %%
# show reversals now with delay in reward (
# reward on working comes at final timestep)
...
reward_func, reward_func_last = (
    get_reward_functions_common(STATES, reward_do, effort_do, reward_completed,
                                cost_completed)
)
T = get_transition_prob(STATES, EFFICACY)
V_opt, policy_opt, Q_values = mdp_algms.find_optimal_policy_prob_rewards(
    STATES, ACTIONS, HORIZON, DISCOUNT_FACTOR_COMMON,
